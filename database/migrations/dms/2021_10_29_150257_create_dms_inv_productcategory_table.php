...
class CreateDmsInvProductcategoryTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('dms_inv_productcategory', function (Blueprint $table) {
            $table->unsignedInteger('iInternalId');
            $table->char('iId', 50)->default('ewid(');
            $table->string('szId', 50)->unique('IX_DMS_INV_ProductCategory');
            $table->string('szName', 50);
            $table->string('szDescription', 200);
            $table->string('szCategoryTypeId', 50)->index('IX_DMS_INV_ProductCategory_1');
    ...
    {
        Schema::dropIfExists('dms_inv_productcategory');
    }
}
