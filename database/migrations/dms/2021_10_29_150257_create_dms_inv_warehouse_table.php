...
class CreateDmsInvWarehouseTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('dms_inv_warehouse', function (Blueprint $table) {
            $table->unsignedInteger('iInternalId')->autoIncrement();
            $table->char('iId', 50)->default('ewid(');
            $table->string('szId', 50)->unique('IX_DMS_INV_Warehouse');
            $table->string('szName', 50);
            $table->string('szDescription', 200);
            $table->string('szBranchId', 50);
            $table->unsignedTinyInteger('bAllowForSalesTransaction');
    ...
    {
        Schema::dropIfExists('dms_inv_warehouse');
    }
}
