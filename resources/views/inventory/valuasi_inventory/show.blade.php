<table class="table table-bordered">
    ...
    <tbody>
        @php
            $totalMasukQty = $totalMasukVal = $totalKeluarQty = $totalKeluarVal = $totalMorphingQty = $totalMorphingVal = 0;
            $saldoAwalQty = 0;
            $saldoAwalVal = 0;
            $saldoAwalPrice = 0;
            if($saldoAwal){
        ...
        @foreach ($detail as $item)
            @php
                ...
                $morphingQty = in_array($item->szTrnId, ['DMSDocStockMorph']) ? $item->decQtyOnHand : 0;                
                $masukVal = $masukQty * $item->price;
                $keluarVal = $keluarQty * $item->price;
                $morphingVal = $morphingQty * $item->price;
                ...
                $saldoAkhirQty += ($masukQty + $keluarQty + $morphingQty);
                $saldoAkhirVal += ($masukVal + $keluarVal + $morphingVal);
            @endphp            
            <tr>
                ...
                <td class="text-right">{{ $exportExcel ? $morphingQty : localNumberFormat($morphingQty,0) }}</td>
                <td class="text-right">{{ $exportExcel ? $item->price : localNumberFormat($item->price,0) }}</td>
                <td class="text-right">{{ $exportExcel ? ($morphingQty * $item->price) : localNumberFormat($morphingQty * $item->price,0) }}</td>
