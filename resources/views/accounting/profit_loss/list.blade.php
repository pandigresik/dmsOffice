            ...
            @foreach ($group->details as $account)
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <button class="btn"><a href="#" data-json='{{ json_encode(["name" => $account->name,"branch_id" => $branch,"startDate" => $startDate, "endDate" => $endDate, "download_xls" => 1, "version" => rand()]) }}' data-url="{{route('accounting.balance.show', $account->code) }}" data-tipe="get" onclick="main.redirectUrl(this);return false"><i class="fa fa-download"></i></a></button>
                    </td>
                    @foreach ($branchMaster as $item)
                        @php                            
